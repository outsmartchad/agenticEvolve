...
| **进化** | 5 阶段流水线：收集 → 分析 → 构建 → 审查 → 自动安装。扫描 12 个来源：GitHub Trending + HN + X/Twitter + Reddit + Product Hunt + Lobste.rs + ArXiv + HuggingFace + BestOfJS + 微信群聊 + Discord 缓存，合成技能 |
...
| **测试** | 423 个自动化测试（423 通过，1 个 xfail）。覆盖：81 个命令处理器集成测试（全部 35+ 处理器）、会话数据库、FTS5 搜索、安全扫描、信号去重、语义搜索、直觉提升、定时解析、费用上限、循环检测、上下文压缩、参数解析 |
...
- **重试工具**：指数退避 + 抖动处理瞬态故障，Telegram 重试辅助函数
- **滚动日志**：RotatingFileHandler（50MB，5 个备份）替代无限增长的日志文件
- **工具循环检测器**：4 种检测模式（通用重复、轮询无进展、乒乓模式、全局断路器）防止失控会话
- **安全自检**：`/doctor` 命令检查环境权限、配置密钥、费用上限、依赖项、沙箱健康状态、数据库完整性
- **诊断事件总线**：类型化事件（消息、用量、会话、循环、心跳），支持 JSONL 输出和状态摘要
- **WhatsApp 命令**：`/cost`、`/status`、`/doctor`、`/help` 现在可在 WhatsApp 上使用
- **语音流水线修复**：长音频分片（48 分钟 → 10 分钟分片）、OGG→WAV 转换、三层去重、语音强制回复、双段响应（转写 + 摘要）
- **安全修复**：CLI/TUI 中的环境变量消毒、动态所有者路径、服务群组的内容包装
...
**WhatsApp 媒体支持**
- 收到的 WhatsApp 图片、文档（PDF、TXT、CSV 等）和音频消息通过 Baileys `downloadMediaMessage` 下载，保存到 `/tmp/`，并传递给 Claude Code 进行分析。带媒体的消息自动升级到 opus 模型。

**自动模型升级**
- 包含数学、编程或逻辑问题的消息通过正则自动检测，路由到 `serve_reasoning_model`（opus）而非默认的 `serve_model`（sonnet）。图片和文件消息也会触发升级。
